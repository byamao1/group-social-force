...
class Agent(object):
    ...
    def selfRepulsion(self, Dfactor=1, Afactor=1, Bfactor=1):
        first = -self.direction * Afactor * self.A_CF * np.exp((self.radius * Dfactor) / (self.B_CF * Bfactor)) * (
                self.radius * Dfactor)
        return first
    ...
    def cohesiveForce(self, other, Dfactor=1, Afactor=1, Bfactor=1):
        ...
        first = Afactor * self.A_CF * np.exp((rij * Dfactor - dij) / (self.B_CF * Bfactor)) * nij * (
                rij * Dfactor - dij)  # *anisoF
        return first
